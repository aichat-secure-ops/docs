...
| Copy from | Information | Paste into |
| :-- | :-- | :-- |
| Google Workspace | Entity ID | HelpAIChat |
| Google Workspace | SSO URL | HelpAIChat |
| Google Workspace | Certificate | HelpAIChat |
| HelpAIChat | ACS URL | Google Workspace |
| HelpAIChat | Audience URI | Google Workspace |
...
4. Enter an app name, such as `AiChat Platform`, to identify the integration.
5. Optionally, upload an AiChat app icon.
6. Click **Continue**.

## Step 3: Add Google identity provider details to AiChat
...
3. Click **Download Certificate**. Open the downloaded certificate and copy its complete contents into **X.509 Certificate** in AiChat.
4. Return to the Google Admin console and click **Continue**.

> Copy the complete certificate, including the `BEGIN CERTIFICATE` and `END CERTIFICATE` lines.

_[Insert image: Google Identity Provider details and AiChat IdP fields]_

## Step 4: Add AiChat service provider details to Google

On the **Service Provider Details** page in Google Workspace:

1. Copy **ACS URL** from AiChat and paste it into the Google Workspace **ACS URL** field.
2. Copy **Audience URI** from AiChat and paste it into the Google Workspace **Entity ID** field.
3. Under **Name ID**, configure the following:
   - **Name ID format:** Leave it as `UNSPECIFIED`.
   - **Name ID:** Select **Basic Information > Primary email**.
4. Click **Continue**.

_[Insert image: Google Service Provider Details]_
...
_[Insert image: Google Workspace attribute mapping]_
...
_[Insert image: Google Workspace user access settings]_
...
- **Magic Link:** Passwordless authentication through email.
...
For more information, refer to Google’s official guide for [setting up a custom SAML app](https://knowledge.workspace.google.com/admin/apps/set-up-your-own-custom-saml-app).
