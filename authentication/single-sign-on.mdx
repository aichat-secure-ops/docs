...
Once the SSO feature is enabled, you can control which sign-in methods members of your organization are allowed to use.

At least one of the following sign-in methods must remain enabled:
...
1. The user opens the organization-specific HelpAIChat login page.
2. The user selects **Sign in with SSO**.
3. HelpAIChat redirects the user to the organization’s configured Identity Provider.
4. The user completes authentication with the Identity Provider.
5. After successful authentication, the user is redirected to the organization dashboard in HelpAIChat.
